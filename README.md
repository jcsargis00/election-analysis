...
The script used for this technical analysis could easily be expanded with minor modification to be used for any election.  The structure is general enough to allow the scope of the analysis to be expanded.  
Examples of other elections the script could be modified and used for include: state wide elections for multiple races (senate, house of represenative, President) with multiple counties, local elections with multiple candidates and seats.  The script could be expaned to be used for a  national election with multiple states and counties within added to the voter data file.  The script is only limited by the data collected in the election_results.cvs file.  
